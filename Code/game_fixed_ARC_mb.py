...
def single_game(env, agent1, agent2, T):
    ...
    for i in range(T):
        ...
        new_m, new_d = agent1.update(action1_index, observations[0])
        agent1.m = new_m
        agent1.d = new_d
        ...
        # if (i+1) % 50 == 0:
        #     print('Current t', i)
        #     print('Current f', agent1.f.flatten())
...
# parameters = [3.35387527, -4.57014207, 0]
# probabilities = expit(
...
# np.savetxt("ARC_mb_frequencies3.csv", agent1_frequencies.T, delimiter=",")

agent1_frequencies = np.loadtxt("ARC_mb_frequencies.csv", delimiter=",")
agent1_frequencies2 = np.loadtxt("ARC_mb_frequencies2.csv", delimiter=",")
agent1_frequencies = np.hstack((agent1_frequencies, agent1_frequencies2))
# plot results
plt.figure(figsize=(7, 5))
for i in range(no_actions):
    plt.semilogx(list(range(1, T+1)),
                 agent1_frequencies[:, i], label=f'$s={action_set[i]}$')
plt.title(f'ARC action frequencies with a fixed opponent $s_j=0.75$')
plt.xlabel('time step')
plt.ylabel('action frequency')
plt.legend()
plt.grid()

plt.savefig('test_game_fixed_ARC_mb_poster.png', dpi=600)
plt.close()
