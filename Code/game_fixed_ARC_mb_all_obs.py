...
from ARC_mb_all_obs import ARCPricingModel
...
def single_game(env, agent1, agent2, T):
    ...
    for i in range(T):
        ...
        if (i+1) % 500 == 0:
            print('Current t', i)
            print('Current f', agent1._f().flatten())
            print('Current m', agent1.m)
            print('Current d', agent1.d)
            print('Current lambda', agent1._lamda())
            print('Current probabilities', agent1.U.flatten())
...
agent1 = ARCPricingModel(candidate_margins=action_set,
                         rho=0.05, beta=0.98, dimension=2)
dummy_action = 0.75
...
plt.savefig('test_game_fixed_ARC_mb_all_obs.png', dpi=600)
plt.close()
