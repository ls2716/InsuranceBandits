...
for it in range(len(observations)):
    ...
    m2, d2 = agent2.update_all_observations_eta(
        action_indices[it], observations[it], eta=eta)
    # print('Update 1')
    # print(m, d)
    print('Update 2')
    print(m2, d2)
...
plt.plot(actions, probabilities2,
         label='posterior with all observations with random walk')
plt.grid()
plt.legend()
plt.scatter(action_set, frequencies)
plt.xlabel('actions')
plt.ylabel('frequencies')
plt.savefig(f'posterior_test_model_based_ARC_allwitheta_vs_single.png')
plt.show()
plt.close()
