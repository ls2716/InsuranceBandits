...
filename = 'epsgreedy_mb_vs_epsgreedy_cl'
title = f'Model-based Eps-Greedy vs Classic Eps-Greedy $\\varepsilon={epsilon}$'
...
bandit_mb = EpsGreedy(
    eps=epsilon, model=stationary_logistic_model)
# Initialize classic epsilon-greedy bandit
bandit_cl = EpsGreedy(
    eps=epsilon, model=stationary_classic_model)
...
start_plot_index = 10
title_mb = f'Model-based Eps-Greedy $\\varepsilon={epsilon}$'
title_cl = f'Classic Eps-Greedy $\\varepsilon={epsilon}$'
...
ut.plot_smooth_reward_history(
    ...
    bandit2_name='Classical Eps-Greedy', foldername=foldername, filename=filename, title=title)
