...
filename = f'eps_greedy_mb_vs_fluc_action_{gamma}'
title = f'Ns-d {gamma} Model-based Eps-Greedy \nvs Fluctuating-Action Agent {mean_action},{half_amplitude},{period}'
...
ut.print_result_to_file(reward_1_sum, reward_2_sum, os.path.join(
    result_folder, filename + '.txt'), f'Ns-d {gamma} Model-based Eps-Greedy', 'Fluctuating-Action Agent')
...
ut.plot_smooth_reward_history(
    dimless_payoff(reward_history), bandit1_name=f'Ns-d {gamma} Model-based Eps-Greedy',
