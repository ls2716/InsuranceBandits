...
from bandits import FluctuatingActionAgent, TwoActionAgent, SequenceAgent
...
def simulate_game(bandit_1_name, bandit_2_name):
    ...
    dimless_reward_history = dimless_payoff(reward_history)

    reward_1_sum = np.sum(reward_history[:, 0])
    reward_2_sum = np.sum(reward_history[:, 1])
    logger.info(
        f'Sum of last 500 rewards for the bandits:'
        + f' {np.sum(dimless_reward_history[-500:, 0])}, {np.sum(dimless_reward_history[-500:, 1])}')
    ut.create_folder(result_folder)
    ut.print_result_to_file(np.sum(dimless_reward_history[:, 0]),
                            np.sum(dimless_reward_history[:, 1]),
                            os.path.join(result_folder, filename + '.txt'),
                            bandit_1_name, bandit_2_name)
    reward_1_sum_500 = np.sum(dimless_reward_history[-500:, 0])
    reward_2_sum_500 = np.sum(dimless_reward_history[-500:, 1])
    ...
    ut.plot_smooth_reward_history(
        dimless_reward_history, bandit1_name=bandit_1_name,
