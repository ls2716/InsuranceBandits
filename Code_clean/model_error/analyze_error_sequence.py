...
def sample_reward_probabilities(S_i, env_params, mixed_actions, action_probs, no_samples=1000):
    ...
    actions[:, 1] = np.random.choice(
        mixed_actions, p=action_probs, size=no_samples, replace=True)
    actions[:, 0] = S_i
...
for i, S_i in enumerate(action_set):
    sample_probs, sample_rewards = \
        sample_reward_probabilities(
            S_i=S_i, env_params=env_params, mixed_actions=[0.1, 0.3], action_probs=[0.5, 0.5], no_samples=5000)
