...
filename = f'ucb_bayes_vs_fluc_action_{gamma}'
title = f'Ns-d {gamma} Classic UCB-Bayes \nvs Fluctuating-Action Agent {mean_action},{half_amplitude},{period}'
...
model = NonStationaryClassicModel(
    variance=variance, candidate_margins=action_set, method='discounting', gamma=gamma)
...
ut.print_result_to_file(reward_1_sum, reward_2_sum, os.path.join(
    result_folder, filename + '.txt'), f'Ns-d {gamma} Classic UCB-Bayes', 'Fluctuating-Action Agent')
...
ut.plot_smooth_reward_history(
    dimless_payoff(reward_history), bandit1_name=f'Ns-d {gamma} Classic UCB-Bayes',
